...
class DeltaAnalyzer(AbstractAnalyzer):
    ...
    def _analyze_table(
        ...
        # 1. Small files
        if self.rule_enabled("delta_small_files"):
            small_file_mb = self.rule_threshold(
                "delta_small_files",
                "small_file_threshold_mb",
                thresholds.small_file_threshold_mb,  # type: ignore[attr-defined]
            )
            if (
                avg_file_mb > 0
                and avg_file_mb < small_file_mb
        ...
        # 2. Stale OPTIMIZE
        if self.rule_enabled("delta_stale_optimize"):
            optimize_stale_days = self.rule_threshold(
                "delta_stale_optimize",
                "optimize_stale_days",
            ...
            if table.last_optimized_at:
                ...
                if days_since_optimize > optimize_stale_days:
                    ...
                    recommendations.append(
                        Recommendation(
                            ...
                            resource_name=table.full_name,
                            title=(
                                f"Table '{table.full_name}' not optimized"
                                f" in {days_since_optimize} days"
                            ),
                            description=(
                ...
                size_gb >= thresholds.optimize_threshold_gb  # type: ignore[attr-defined]
                and table.history_error is None
            ):
                savings = max(Decimal(str(size_gb * 0.023 * 0.3)), Decimal("5"))
                recommendations.append(
                    Recommendation(
        ...
        # 3. Stale VACUUM
        if self.rule_enabled("delta_stale_vacuum"):
            vacuum_stale_days = self.rule_threshold(
                "delta_stale_vacuum",
                "vacuum_stale_days",
                ...
                if days_since_vacuum > vacuum_stale_days:
                    ...
                    recommendations.append(
                        Recommendation(
                            ...
                            resource_name=table.full_name,
                            title=(
                                f"Table '{table.full_name}' not vacuumed"
                                f" in {days_since_vacuum} days"
                            ),
                            description=(
        ...
        # 4. Over-optimized
        if self.rule_enabled("delta_over_optimized"):
            over_optimize_threshold = self.rule_threshold(
                "delta_over_optimized",
                "over_optimize_threshold",
                thresholds.over_optimize_threshold,  # type: ignore[attr-defined]
            )
            optimize_count = table.optimize_count_30d or 0
            if optimize_count > over_optimize_threshold:
                wasted_runs = optimize_count - int(over_optimize_threshold)
                ...
                recommendations.append(
                    Recommendation(
                        ...
                        resource_name=table.full_name,
                        title=(
                            f"Table '{table.full_name}' optimized {optimize_count} times in 30 days"
                        ),
                        description=(
                        ...
                        current_state={
                            ...
                            "threshold": int(over_optimize_threshold),
                        },
        ...
        if self.rule_enabled("delta_migrate_to_liquid_clustering"):
