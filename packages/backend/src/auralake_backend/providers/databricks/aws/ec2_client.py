...
class EC2Client:
    ...
    def describe_instances(self, filters: list[dict] | None = None) -> list[dict]:
        try:
            ...
            raise APIError("databricks", f"EC2 describe-instances failed: {exc}") from exc
    ...
    def get_spot_price_history(
        ...
        try:
            ...
            raise APIError("databricks", f"EC2 spot price history failed: {exc}") from exc

    def describe_volumes(self, filters: list[dict] | None = None) -> list[dict]:
        try:
            ...
            raise APIError("databricks", f"EC2 describe-volumes failed: {exc}") from exc
