...
from auralake_shared.core.exceptions import DABParserError
from auralake_shared.models.config import AuraLakeConfig
from auralake_shared.models.dab import (
    BundleInfo,
    DABBundle,
    DABClusterConfig,
    DABDiff,
...
from auralake_shared.providers.base import AbstractConfigFormat
...
class DABParser:
    ...
    def add_job_to_cluster(self, file_path: Path, job_key: str, shared_cluster_key: str) -> DABDiff:
        ...
        return self.modify_job(
            ...
            {
                "tasks": [
                    {"existing_cluster_id": f"${{resources.clusters.{shared_cluster_key}.id}}"}
                ],
            },
        )
