...
2. Check the allocation basis before comparing requesters or quoting a cost per million
   tokens.
3. Use token volumes for usage analysis even when dollar allocation is unavailable.
4. Investigate provisioned endpoints with the associated efficiency evidence rather than
   attributing idle capacity to a requester.

See [GOLD views and metrics](../concepts/gold-views.md) for shared metric semantics and
[MCP tools](../reference/mcp-tools.md) for discovery/query workflow.
