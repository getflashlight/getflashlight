...
underutilized/idle is honest; `owner_user` is a best-effort heaviest-user hint, or the
hard `single_user_name` fact when the cluster is single-user), and the **job** for jobs.
SQL warehouses expose no per-entity utilization (`utilization_pct` = NULL) → never
flagged underutilized; their waste is query-pattern health, per-user cost concentration,
cadence, and serverless-vs-classic pricing signals instead. Model Serving waste is
*idle-provisioned*, not utilization-based.
*(Validated against a live warehouse 2026-06-28 — see Build order.)*

**Classic vs. serverless notebooks — not a shared-vs-single-user distinction.** Whether
notebook-level cost is *measurable* depends on classic vs. serverless billing, not
access mode: classic all-purpose clusters (shared or single-user) meter cost at the
**cluster** level — `system.billing.usage` carries no notebook identity for them at all,
so notebook-level attribution there would need a new telemetry source
(`system.access.audit` notebook attach/command events, to time-allocate cluster cost) —
an estimate, not a measurement. That stays **BLOCKED** (`classic_notebook_attribution` in
`waste_rules.py`), not built. Serverless notebooks get `usage_metadata.notebook_id`/
`notebook_path` populated directly (already proven for FOCUS resource naming in
`databricks_focus_1_3.sql`), so their per-notebook `billed_cost` is a real sum today.
...
Investigated 2026-07-11 (live `DESCRIBE`/`SELECT` against a production warehouse), when
extending `sql_warehouse_disk_spill` — recorded here rather than silently left as a gap.

1. **No spill/shuffle signal for classic JOBS / ALL_PURPOSE Spark execution.** Confirmed
   via `SHOW TABLES IN system.compute` / `system.lakeflow`: no system table exposes Spark
   stage-level metrics for arbitrary DataFrame/RDD code. `system.compute.node_timeline`
   has only CPU/memory/network/disk-free columns; `system.lakeflow.job_run_timeline` has
   only duration/status/trigger metadata. `system.query.history` (the source of the
   `sql_warehouse` spill/shuffle signal) only covers statements executed via the SQL/
   Spark-Connect path — `compute.type` is never `JOBS`/`ALL_PURPOSE` on that table. The
   two real paths outside system tables are both deliberately out of scope for now:
   - Databricks' native "Compute metrics" UI *does* show shuffle read/write for jobs/
     all-purpose compute, but it's UI-only — no documented REST API or system table
     backs it. Automating it means reverse-engineering an undocumented internal API,
     which breaks this project's vendored-and-validated-query convention.
   - Spark event log replay: cluster log delivery is off by default (an org-wide
     cluster-policy change to turn on), logs land as raw JSON files in DBFS/S3, and the
     only documented access is a UI notebook replay — not SQL, not an API. A real pull
     would mean a new JSON-parsing ingestion pipeline, not a `databricks_efficiency.sql`
     extension.
   - A self-hosted `spark-metrics` → Prometheus Pushgateway stack (community pattern)
     would get real per-cluster shuffle/spill, but needs cluster init-script changes
     org-wide, new metrics infrastructure, and a new non-SQL connector architecture
     (pull from Prometheus's HTTP API instead of a Databricks warehouse) — a distinct
     initiative, not an extension of this connector.

   **Partial mitigation added 2026-07-12**: `possible_memory_pressure` and
   `possible_heavy_shuffle` in `waste_rules.py` approximate spill/shuffle from
   `system.compute.node_timeline` columns already joined for CPU/mem utilization —
   `cpu_wait_percent` (I/O wait is spill's textbook symptom), `mem_swap_percent`
   (memory exhaustion), `disk_free_bytes_per_mount_point` at `/local_disk0` (spill's
   actual write target — not the OS root partition, which is always small regardless
   of spill activity), and `network_sent_bytes`/`network_received_bytes` (shuffle is
   inter-executor network I/O). These are proxies, not measurements — always
   `'candidate'` confidence, `recoverable_cost_sql="0"` (a flag for review, no $
   claim), gated by `avg_run_seconds >= 300` for job entities (a materiality filter:
   a short job with a scary-looking proxy reading isn't worth optimizing) that fails
   closed on untracked runs rather than guessing. Thresholds were grounded in a
   30-day percentile scan of this account's job/all-purpose node_timeline rows (see
   inline comments in `databricks_efficiency.sql`'s `util`/`runs` CTEs for the exact
   numbers) — not picked blind. This narrows the gap; it does not close it — a real
   spill event with low CPU wait, or shuffle that doesn't saturate the network,
   would still go undetected.
2. **DLT/Lakeflow declarative pipeline (serverless) compute is entirely unattributed in
   the efficiency/waste plane** — not just missing spill/shuffle, missing as an entity at
   all. Confirmed live: this usage bills under `billing_origin_product IN ('SQL',
   'DATABASE')` with `usage_metadata.dlt_pipeline_id` populated but `warehouse_id`/
   `cluster_id`/`job_id` all NULL, so none of the 5 `UNION ALL` branches in
   `databricks_efficiency.sql` pick it up (each requires one of those four IDs). The
   FOCUS/TCO cost pull is unaffected — `databricks_focus_1_3.sql` sums `billing.usage`
   unconditionally, so this spend is billed correctly; it's specifically invisible to
   waste classification. Closing this needs a new `EntityType` + a 6th branch keyed on
   `dlt_pipeline_id`, with its own `billed_cost` attribution — a new-entity-type feature,
   not a SQL tweak. Deferred: sampled 30 days of this billing line and found zero spill,
   zero shuffle — the signal doesn't currently justify the lift. Revisit if DLT pipeline
   spend grows or a spill/shuffle complaint actually shows up on this line.
...
4. **Phase 1b** — Model Serving: extend the connector's emit to `entity_type='endpoint'`
   ...
   view, just new rows.
5. **Done** — `sql_warehouse` disk spill (`system.query.history.spilled_local_bytes` →
   `cause_detail.spill_query_count`/`spilled_bytes` → `sql_warehouse_disk_spill` in
   `waste_rules.py`, validated live 2026-07-11: 78/66,709 queries spilled, worst case
   ~21 TB); autoscale/oversize (`waste_rules.py`).
6. **Later** — serverless placement; BigQuery/Snowflake mappers (each = a connector
   emitting the same `EfficiencyRecord`, zero GOLD change).
