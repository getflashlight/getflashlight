...
No network: writes a tiny FOCUS CSV and runs the same DuckDB path `flashlight
...
def test_seed_maps_and_builds_gold(lake_home, tmp_path) -> None:  # type: ignore[no-untyped-def]
    ...
    from flashlight.lake import seed
...
def test_sample_cleanup_removes_all_data(lake_home, tmp_path) -> None:  # type: ignore[no-untyped-def]
    from flashlight.lake import paths, runlog, seed
    from flashlight.sample import SAMPLE_CONNECTOR, cleanup
