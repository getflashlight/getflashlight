...
def test_build_export_request_shape() -> None:
    ...
    assert req["Name"] == "flashlight-focus"
...
def test_state_roundtrip_and_precedence(tmp_path, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    monkeypatch.setenv("FLASHLIGHT_STATE_DIR", str(tmp_path))
    from flashlight.ingest.aws_export_setup import load_state, resolved_targets, save_state
...
def test_print_export_shows_status_and_last_run(capsys) -> None:  # type: ignore[no-untyped-def]
    from flashlight.ingest.aws_export_setup import _print_export
    ...
    _print_export("arn:aws:bcm-data-exports:…:export/flashlight-focus", export, status)
...
def test_prefix_eq_ignores_slashes() -> None:
    from flashlight.ingest.aws_export_setup import _prefix_eq
...
def test_human_bytes() -> None:
    from flashlight.ingest.aws_export_setup import _human_bytes
...
def test_find_export_by_name() -> None:
    from flashlight.ingest.aws_export_setup import _find_export_by_name

    client = _FakeBcmClient(
        [
            {"ExportName": "other", "ExportArn": "arn:other"},
            {"ExportName": "flashlight-focus", "ExportArn": "arn:focus"},
        ]
    )
    assert _find_export_by_name(client, "flashlight-focus") == "arn:focus"
...
def test_perform_create_export_warns_on_duplicate_name(monkeypatch, capsys) -> None:  # type: ignore[no-untyped-def]
    import flashlight.ingest.aws_export_setup as mod

    # An export with this name already exists → creating another should warn.
    client = _FakeBcmClient([{"ExportName": "flashlight-focus", "ExportArn": "arn:existing"}])
...
def test_current_export_destination_reads_live_values(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    client = _FakeBcmClient(
        [{"ExportName": "flashlight-focus", "ExportArn": "arn:focus"}], current=current
    )
    monkeypatch.setattr(mod, "_bcm_client", lambda _defaults: client)

    dest = mod.current_export_destination("flashlight-focus", "/nonexistent.yml")
...
def test_perform_update_export_confirm_false_aborts(monkeypatch, capsys) -> None:  # type: ignore[no-untyped-def]
    ...
    client = _FakeBcmClient([{"ExportName": "flashlight-focus", "ExportArn": "arn:focus"}])
    monkeypatch.setattr(mod, "_bcm_client", lambda _defaults: client)
...
def test_perform_delete_export_dry_run_does_not_delete(monkeypatch, capsys) -> None:  # type: ignore[no-untyped-def]
    ...
    mod.perform_delete_export(apply=False, name="flashlight-focus", connections="/nonexistent.yml")
    assert client.deleted is None  # dry run
    assert "DRY RUN" in capsys.readouterr().out

    mod.perform_delete_export(apply=True, name="flashlight-focus", connections="/nonexistent.yml")
