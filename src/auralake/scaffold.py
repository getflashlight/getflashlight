"""``auralake init`` — scaffold the lake home with a starter config.
...
It does **not** bundle sample data — `auralake sample` downloads the FinOps FOCUS
...
# Auralake connectors. Uncomment and fill in the sources you want to ingest.
...
def scaffold(force: bool = False) -> None:
    ...
    typer.echo("\nNext steps:")
    typer.echo("  auralake sample            # download the FOCUS sample + seed it (no config)")
    typer.echo(f"  # or edit {conn} to add your sources, then: auralake ingest")
