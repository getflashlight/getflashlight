...
from auralake.core.exceptions import GitIntegrationError
from auralake.core.logging import get_logger
from auralake.models.config import GitHubConfig
...
class RepoManager:
    ...
    def ensure_repo(self) -> Path:
        ...
        if clone_dir.exists() and (clone_dir / ".git").exists():
            ...
            url = f"https://{token}@github.com/{self._config.repo}.git" if token else f"https://github.com/{self._config.repo}.git"
            try:
    ...
    def _pull(self) -> None:
        ...
        try:
            ...
            logger.warning("pull_failed", msg="Could not fast-forward pull; proceeding with current state")
