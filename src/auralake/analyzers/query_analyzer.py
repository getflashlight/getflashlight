...
from auralake.agent.plan_parser import PlanParser
    ...
    def analyze(self) -> AnalysisResult:
        ...
        for query in sorted_queries[:10]:
            ...
            if duration_ms > 300_000:  # > 5 minutes
                recommendations.append(Recommendation(
                    ...
                    description=(query.get("query_text", "")[:200] + "...") if len(query.get("query_text", "")) > 200 else query.get("query_text", ""),
                ...
                if plan_text:
                    ...
                    for ap in plan.anti_patterns:
                        recommendations.append(Recommendation(
                            ...
                            risk_level=RiskLevel.MEDIUM if ap.severity == "high" else RiskLevel.LOW,
                            ...
                            estimated_monthly_savings_usd=Decimal("0"),
                            savings_confidence=SavingsConfidence.LOW,
                            evidence={"anti_pattern": ap.type, "recommendation": ap.recommendation},
                        ))
            except Exception:
                continue
        ...
        return AnalysisResult(
            ...
            summary={
                ...
                "expensive_queries": len([r for r in recommendations if r.type == "query_expensive"]),
                "anti_patterns_found": len([r for r in recommendations if r.type.startswith("query_") and r.type != "query_expensive"]),
            },
        )
