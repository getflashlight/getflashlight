...
from auralake.models.jobs import ConsolidationGroup, JobProfile
...
class JobAnalyzer(AbstractAnalyzer):
        ...
        for job in jobs:
            ...
            if len(failed) >= 3:
                recommendations.append(Recommendation(
                    ...
                    estimated_monthly_savings_usd=Decimal(str(job.avg_dbu_cost * 30)) if job.avg_dbu_cost else Decimal("0"),
                    savings_confidence=SavingsConfidence.MEDIUM,
                    evidence={"failed_runs": len(failed), "total_runs": len(runs)},
                ))

        # Job consolidation analysis
        groups = self._find_consolidation_groups(jobs, thresholds.min_group_size, thresholds.max_group_size)
        for group in groups:
    ...
    def _find_consolidation_groups(
        self, jobs: list[JobProfile], min_size: int, max_size: int,
