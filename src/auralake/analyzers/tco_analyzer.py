...
from auralake.models.billing import TCORecord
from auralake.models.recommendations import AnalysisResult
    ...
    def analyze(self) -> AnalysisResult:
        ...
        for m in mappings:
            if m.platform_resource_type == "cluster":
                cluster_to_ec2.setdefault(
                    m.platform_resource_id, []
                ).append(m.infra_resource_id)
        ...
        for c in compute_costs:
            ec2_cost_by_resource[c.resource_id] = (
                ec2_cost_by_resource.get(c.resource_id, Decimal("0"))
                + c.cost_usd
            )

        all_cluster_ids = set(breakdown.by_cluster.keys()) | set(
            cluster_to_ec2.keys()
        )
        tco_records = []
        for cluster_id in all_cluster_ids:
            ...
            for ec2_id in cluster_to_ec2.get(cluster_id, []):
                ec2_cost += ec2_cost_by_resource.get(
                    ec2_id, Decimal("0")
                )
        ...
        total_dbu = sum(
            (r.dbu_cost for r in tco_records), Decimal("0")
        )
        total_ec2 = sum(
            (r.ec2_cost for r in tco_records), Decimal("0")
        )
        total_storage = sum(
        ...
        return AnalysisResult(
            ...
            summary={
                ...
                "total_tco_usd": str(
                    total_dbu + total_ec2 + total_storage + total_transfer
                ),
