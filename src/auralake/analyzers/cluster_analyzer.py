...
from auralake.models.recommendations import AnalysisResult, Recommendation, RiskLevel, SavingsConfidence
...
class ClusterAnalyzer(AbstractAnalyzer):
    ...
    def analyze(self) -> AnalysisResult:
        ...
        for cluster in clusters:
            ...
            # Right-sizing check
            if utilization.avg_cpu_percent > 0 and utilization.avg_cpu_percent < thresholds.cpu_utilization_low:
                ...
                if savings >= thresholds.min_savings_dollars:
                    recommendations.append(Recommendation(
                        ...
                        description=(
                            f"Avg CPU: {utilization.avg_cpu_percent:.1f}% (threshold: {thresholds.cpu_utilization_low}%). "
                            f"Consider reducing workers or instance size."
                        ),
                        current_state={
            ...
            # Idle cluster check
            idle_threshold = self.context.config.thresholds.idle_resources.idle_cluster_minutes
            if cluster.state == "RUNNING" and utilization.idle_hours > (idle_threshold / 60):
                recommendations.append(Recommendation(
                    ...
                    description=f"Cluster has been idle for {utilization.idle_hours:.1f} hours. Consider terminating.",
                    current_state={"state": cluster.state, "idle_hours": utilization.idle_hours},
            ...
            if cluster.autotermination_minutes is None or cluster.autotermination_minutes == 0:
                recommendations.append(Recommendation(
                    ...
                    description="Cluster has no autotermination configured. Enable to prevent idle cost waste.",
            ...
            if not cluster.spot_enabled and cluster.num_workers > 0:
                recommendations.append(Recommendation(
                    ...
                    description=f"Cluster has {cluster.num_workers} workers on on-demand. Spot can save 50-90%.",
                    current_state={"spot_enabled": False},
                    recommended_state={"spot_enabled": True, "spot_fallback": True},
                    estimated_monthly_savings_usd=Decimal(str(utilization.total_cost_usd * 0.5)),
                    savings_confidence=SavingsConfidence.MEDIUM,
                ))
