...
from auralake.providers.base import AbstractComputeClient
...
class DatabricksComputeClient(AbstractComputeClient):
    ...
    def list_clusters(self) -> list[ClusterInfo]:
        try:
            ...
            raise APIError(
                "databricks", f"Failed to list clusters: {exc}"
            ) from exc
    ...
    def get_cluster(self, cluster_id: str) -> ClusterInfo:
        try:
            ...
            raise APIError(
                "databricks", f"Failed to get cluster {cluster_id}: {exc}"
            ) from exc
    ...
    def _to_cluster_info(c) -> ClusterInfo:
        ...
        return ClusterInfo(
            ...
            cluster_source=(
                str(c.cluster_source.value) if c.cluster_source else None
            ),
