...
from auralake.models.billing import CostBreakdown, CostRecord, PriceRecord
from auralake.models.config import DatabricksConfig
from auralake.providers.base import AbstractCostClient
...
class DatabricksCostClient(AbstractCostClient):
    ...
    def get_usage(
        ...
        try:
            ...
            raise APIError(
                "databricks", f"Failed to query billing: {exc}"
            ) from exc
    ...
    def get_pricing(self) -> list[PriceRecord]:
        ...
        try:
            ...
            raise APIError(
                "databricks", f"Failed to query pricing: {exc}"
            ) from exc

    def get_cost_breakdown(self, start: date, end: date) -> CostBreakdown:
        records = self.get_usage(
            start, end, group_by=["sku_name", "cluster_id", "job_id"]
        )
        ...
        for r in records:
            ...
            if r.cluster_id:
                by_cluster[r.cluster_id] = (
                    by_cluster.get(r.cluster_id, Decimal("0")) + r.cost_usd
                )
            if r.job_id:
                by_job[r.job_id] = (
                    by_job.get(r.job_id, Decimal("0")) + r.cost_usd
                )
        ...
        for ws_name, ws_config in self._config.workspaces.items():
            if ws_config.sql_warehouse_id:
                ...
                if result.result and result.result.data_array:
                    columns = [
                        col.name
                        for col in (result.manifest.schema.columns or [])
                    ]
                    return [
                ...
                return []
        raise APIError(
            "databricks", "No workspace with sql_warehouse_id configured"
        )
