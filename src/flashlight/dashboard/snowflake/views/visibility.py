        ...
        with ui.row().classes("w-full gap-4"):
            ...
            with ui.column().classes("flex-[2]"):
                with chrome.panel():
                    ...
                    # Actual area fill (solid)
                    fig_fc.add_trace(go.Scatter(
                        x=actual["month"], y=actual["tco"],
                        mode="lines", name="Actual ($)",
                        line=dict(color="#1b6b7a", width=2),
                        fill="tozeroy",
                        fillcolor="#1b6b7a",
                        hovertemplate="<b>%{x|%b %Y}</b><br>"
                                      "TCO: $%{y:,.0f}<extra></extra>",
                    ))
                    # Forecast area fill (solid, different color)
                    bridge = actual.iloc[[-1]][["month", "tco"]].copy()
                    forecast_with_bridge = pd.concat(
                        [bridge, forecast[["month", "tco"]]], ignore_index=True)
                    fig_fc.add_trace(go.Scatter(
                        x=forecast_with_bridge["month"],
                        y=forecast_with_bridge["tco"],
                        mode="lines", name="Forecast ($)",
                        line=dict(color="#d96a3b", width=2),
                        fill="tozeroy",
                        fillcolor="#d96a3b",
                        hovertemplate="<b>%{x|%b %Y}</b><br>"
                    ...
                    chrome.style_fig(fig_fc, height=400, has_legend=True,
                                     currency_axis="y")
                    all_tco = forecast_df["tco"].values
                    y_min = int(min(all_tco) * 0.85 / 50000) * 50000
                    y_max = int(max(all_tco) * 1.1 / 50000 + 1) * 50000
            ...
            with ui.column().classes("flex-1"):
                with chrome.panel():
                    ...
                    table_df["TCO Actual"] = table_df.apply(
                        lambda r: f"${r['tco']:,.0f}"
                        if r["type"] == "Actual" else "—", axis=1)
                    table_df["TCO Forecast"] = table_df.apply(
                        lambda r: f"${r['tco']:,.0f}"
                        if r["type"] == "Forecast" else "—", axis=1)

                    columns = [
                        {"name": "Month", "label": "Month",
                         "field": "Month", "align": "left"},
                        {"name": "Actual", "label": "Actual",
                         "field": "TCO Actual", "align": "right"},
                        {"name": "Forecast", "label": "Forecast",
                         "field": "TCO Forecast", "align": "right"},
                    ]
                    rows = table_df[["Month", "TCO Actual",
                                     "TCO Forecast"]].to_dict("records")
                    ui.table(
                        columns=columns, rows=rows,
                    ).props("dense flat hide-pagination").classes(
                        "w-full"
                    ).style(f"background:{chrome.SURFACE};"
