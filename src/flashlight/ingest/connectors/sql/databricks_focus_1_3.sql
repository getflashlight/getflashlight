...
-- and feed its FOCUS-columned output through flashlight.ingest.connectors._focus_map.
--
...
usage_with_pricing AS (
  SELECT
    u.record_id,
    u.record_type,  -- Flashlight addition: ORIGINAL | RETRACTION | RESTATEMENT (correction model)
    u.account_id,
    u.workspace_id,
    w.workspace_name,
    u.sku_name,
...
SELECT
  ...
  -- Flashlight additions: identity of the physical usage record + its correction
