...
Flashlight *consumes* a FOCUS export from S3; it does not create one. This command
...
from flashlight.ingest.config import env
...
def default_query_statement() -> str:
    """The FOCUS projection Flashlight ingests. Override with --query-statement."""
...
def perform_create_export(
    *,
    ...
    """Build (and optionally create) the FOCUS export. Backs ``flashlight aws create-export``.
    ...
    if existing:
        print(
            ...
            "To modify it in place instead, abort and run: flashlight aws update-export"
        )
    ...
    print(f"\nCreated export: {arn}")
    print("First data delivery can take up to 24 hours. Then run: flashlight ingest")
    ...
    *,
    ...
    if bucket_changed:
        print(
            ...
            f"  flashlight aws bucket-policy --bucket {bucket}"
        )
...
def bucket_policy_hint(bucket: str, connections: str | None) -> str:
    ...
    return (
        ...
        f"Or let Flashlight do it:  flashlight aws bucket-policy --bucket {bucket}"
    )
...
def print_bucket_policy(bucket: str, connections: str | None) -> None:
    ...
    print(
        ...
        f"--policy file://policy.json\nor: flashlight aws bucket-policy --bucket {bucket}"
    )
...
def _state_path() -> Path:
    base = os.environ.get("FLASHLIGHT_STATE_DIR") or str(Path.home() / ".config" / "flashlight")
