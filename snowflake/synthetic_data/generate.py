"""Generate synthetic Snowflake ACCOUNT_USAGE datasets for a $3M/year demo account.

Produces one Parquet file per source view, modeling a mixed-workload account:
  - 14 warehouses across ETL, BI, ML/AI, dev, streaming
  - ~750K credits/year (~62.5K/month, ~$4/credit = $3M/year)
  - 30 days of data
...
DAYS = (date.today() - date(2026, 1, 1)).days or 203  # Jan 1 2026 to today
START_DATE = date(2026, 1, 1)
CREDIT_PRICE = 4.00  # $/credit — yields ~$3M/year at 750K credits
...
MONTHLY_CREDITS = 84000  # scaled to compensate for hourly traffic weights → ~$3M/year
...
def generate_warehouse_metering_history() -> pd.DataFrame:
    ...
    for day_offset in range(DAYS):
        ...
        # Monthly growth: ~1.5%/month increase (≈$4-5K/month at $320K base)
    ...
    print(f"  warehouse_metering_history: {len(df)} rows, {df['credits_used'].sum():.0f} total credits")
    return df
...
def generate_warehouse_load_history() -> pd.DataFrame:
    ...
    print(f"  warehouse_load_history: {len(df)} rows")
    return df
...
def generate_query_history() -> pd.DataFrame:
    ...
    print(f"  query_history: {len(df)} rows")
    return df
...
def generate_query_attribution_history(query_df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"  query_attribution_history: {len(result)} rows, {result['credits_attributed_compute'].sum():.0f} credits")
    return result


def generate_storage_usage() -> pd.DataFrame:
    """Daily account storage for full period — growing ~1,100 TB total (~10% of spend)."""
    rows = []
    base_storage = 900.0 * (1024**4)  # 900 TB in bytes
    base_stage = 120.0 * (1024**4)  # 120 TB stages
    base_failsafe = 60.0 * (1024**4)  # 60 TB failsafe
    ...
    for day_offset in range(DAYS):
        ...
        rows.append({
            ...
            "hybrid_table_storage_bytes": int(20 * (1024**4) * np.random.uniform(0.9, 1.1)),
            "archive_storage_cool_bytes": int(40 * (1024**4) * growth),
            "archive_storage_cold_bytes": int(20 * (1024**4) * growth),
        })
    df = pd.DataFrame(rows)
    pq.write_table(pa.Table.from_pandas(df), OUTPUT_DIR / "storage_usage.parquet")
    print(f"  storage_usage: {len(df)} rows")
    return df


def generate_table_storage_metrics() -> pd.DataFrame:
    """Top 200 tables by storage. Realistic range: 100 MB to 500 TB max."""
    databases = ["RAW", "ANALYTICS", "ML_FEATURES", "REPORTING", "STAGING"]
    schemas = ["PUBLIC", "CORE", "MARTS", "STAGING", "ML", "FEATURES"]
    rows = []
    # Cap at 500 TB = 500 * 1024^4 bytes
    max_bytes = 500 * (1024 ** 4)
    for i in range(200):
        # lognormal(25, 1.8) gives a range of ~100MB to ~500TB
        active = min(int(np.random.lognormal(25, 1.8)), max_bytes)
        rows.append({
    ...
    print(f"  table_storage_metrics: {len(df)} rows")
    return df
...
def generate_tag_references() -> pd.DataFrame:
    ...
    print(f"  tag_references: {len(df)} rows "
          f"({tagged_wh}/{len(wh_names)} WH tagged, {full_count} fully tagged)")
    return df
...
def generate_warehouse_events_history() -> pd.DataFrame:
    ...
    print(f"  warehouse_events_history: {len(df)} rows")
    return df
...
def generate_metering_history() -> pd.DataFrame:
    ...
    print(f"  metering_history (AI + managed): {len(df)} rows, "
          f"{total_ai:.0f} AI credits, {total_managed:.0f} managed svc credits")
    return df
...
def generate_cortex_ai_functions_usage() -> pd.DataFrame:
    ...
    print(f"  cortex_ai_functions_usage_history: {len(df)} rows")
    return df
...
def generate_cortex_search_daily_usage() -> pd.DataFrame:
    ...
    print(f"  cortex_search_daily_usage_history: {len(df)} rows")
    return df
...
def generate_metering_daily_history() -> pd.DataFrame:
    ...
    print(f"  metering_daily_history: {len(df)} rows")
    return df
...
def generate_serverless_services_data() -> None:
    ...
    print(f"  automatic_clustering_history: {len(df)} rows")
    ...
    print(f"  pipe_usage_history: {len(df)} rows")
    ...
    print(f"  serverless_task_history: {len(df)} rows")
    ...
    print(f"  data_transfer_history: {len(df)} rows")  # noqa: E501
...
def generate_shadow_waste_data() -> None:
    ...
    for wh_name, size, cph, wtype, pct in WAREHOUSES:
        ...
        if wtype == "dev":
            idle_hours = int(np.random.uniform(200, 500))
            idle_credits = idle_hours * np.random.uniform(1.5, 3.0)
        elif wtype in ("bi", "analytics"):
            idle_hours = int(np.random.uniform(50, 150))
            idle_credits = idle_hours * np.random.uniform(2.0, 6.0)
        else:
            idle_hours = int(np.random.uniform(10, 60))
    ...
    for wh_name, size, cph, wtype, pct in WAREHOUSES:
        if wtype in ("dev", "bi") and random.random() < 0.5:
            ...
            over_credits = np.random.uniform(50, 300)
            rows.append({
    ...
    df = pd.DataFrame(rows)
    pq.write_table(pa.Table.from_pandas(df), OUTPUT_DIR / "hidden_waste_compute.parquet")
    print(f"  hidden_waste_compute: {len(df)} rows, ${df['wasted_cost_usd'].sum():,.0f}")
    ...
    for table in stale:
        tb = np.random.uniform(5, 40)  # 5-40 TB stale tables
    ...
    for table in ["ANALYTICS.CORE.FACT_ORDERS", "RAW.PUBLIC.RAW_CLICKSTREAM"]:
        tb = np.random.uniform(10, 30)  # 10-30 TB time travel excess
    ...
    for i in range(3):
        tb = np.random.uniform(8, 25)  # 8-25 TB abandoned clones
    ...
    df = pd.DataFrame(rows)
    pq.write_table(pa.Table.from_pandas(df), OUTPUT_DIR / "hidden_waste_storage.parquet")
    print(f"  hidden_waste_storage: {len(df)} rows, ${df['monthly_cost_usd'].sum():,.0f}/mo")

    # ── AI/Cortex Shadow Waste (6 patterns from snowflake-ai-finops) ──────
    rows = []
    # P1: Over-sized models
    for func, model, task, calls, cr, alt, sav in [
        ("AI_COMPLETE", "claude-3-5-sonnet", "sentiment", 850, 4680, "AI_SENTIMENT", "50-70%"),
        ("AI_COMPLETE", "mistral-large2", "classification", 620, 2280, "AI_CLASSIFY", "50-75%"),
        ("AI_COMPLETE", "llama3.1-70b", "extraction", 430, 1580, "AI_EXTRACT", "30-60%"),
    ]:
        rows.append({
    ...
    for func, cause, calls, cr in [
        ("AI_SENTIMENT", "hourly_no_incremental", 12000, 180),
        ("AI_COMPLETE", "notebook_rerun", 3500, 420),
        ("AI_CLASSIFY", "retry_on_success", 2800, 95),
    ]:
        rows.append({
    ...
    rows.append({
        ...
        "calls_30d": 15000,
        "actual_cost_usd": 320.0 * CREDIT_PRICE * 3.0,
        "wasted_credits": 320.0,
        "wasted_cost_usd": 320.0 * CREDIT_PRICE,
        "recommendation": "Trim prompts — avg 200 tokens filler/call",
    })
    # P4: Idle Cortex Search
    for svc, cr in [("ABANDONED_POC_SEARCH", 45), ("OLD_DEMO_SEARCH", 28)]:
        rows.append({
    ...
    rows.append({
        ...
        "calls_30d": 85,
        "actual_cost_usd": 250.0 * CREDIT_PRICE * 2.0,
        "wasted_credits": 250.0,
        "wasted_cost_usd": 250.0 * CREDIT_PRICE,
    ...
    rows.append({
        ...
        "calls_30d": 2200,
        "actual_cost_usd": 380.0 * CREDIT_PRICE,
        "wasted_credits": 380.0,
        "wasted_cost_usd": 380.0 * CREDIT_PRICE,
        "recommendation": "Revoke AI access from DEV_ROLE in prod",
    })
    df = pd.DataFrame(rows)
    pq.write_table(pa.Table.from_pandas(df), OUTPUT_DIR / "hidden_waste_ai.parquet")
    print(f"  hidden_waste_ai: {len(df)} rows, ${df['wasted_cost_usd'].sum():,.0f}")


def main() -> None:
    print("Generating synthetic Snowflake ACCOUNT_USAGE data for $3M/year demo account...")
    print(f"  Account: {ACCOUNT}")
    print(f"  Period: {START_DATE} to {START_DATE + timedelta(days=DAYS-1)}")
    print(f"  Monthly credits target: {MONTHLY_CREDITS:,}")
    print(f"  Annual cost target: ${MONTHLY_CREDITS * 12 * CREDIT_PRICE:,.0f}")
    print()
